...
import { loanTenure, residencyType } from "../../../../configs/selectorConfigs";
    ...
    <>
      <div className="-mb-2.5 -ml-2.5 flex items-center space-x-2.5">
        <TbSquareRoundedChevronLeftFilled
          onClick={() => navigate("/")}
          className="text-3xl cursor-pointer text-cyan-300"
        />
          ...
          <ul className="grid grid-cols-2 sm:grid-cols-3 gap-2.5 mt-2">
            {loanTenure.map((i) => (
              <li
                key={i}
                onClick={() =>
                  dispatch(setFormData({ ...formData, loanTenure: i }))
                }
                style={{
                  borderColor: formData.loanTenure === i && "#06b6d4",
                }}
                className={`w-full border border-slate-300 rounded-lg flex items-center justify-between p-2.5 hover:border-cyan-500 cursor-pointer ${
                  formData.loanTenure === i && "bg-cyan-50"
                }`}
              >
                <span>{i}</span>
                {formData.loanTenure === i ? (
