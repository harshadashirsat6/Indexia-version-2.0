...
const StudentDetails = ({ formik }) => {
  ...
  //student pan card number validation
  const studentPancardValidation = (val) => {
    if (val) {
      if (val.length !== 10) {
        return (
          <span className="text-red-500 text-xs font-bold">
            Pan card number should be 10 characters
          </span>
        );
      } else if (!/^[a-zA-Z]{5}\d{4}[a-zA-Z]$/.test(val)) {
        return (
          <span className="text-red-500 text-xs font-bold">
  ...
  return (
    <>
      ...
      <div className="col-span-1 sm:col-span-2">
        ...
        {formik.values.panCardNum &&
          studentPancardValidation(formik.values.panCardNum)}
      </div>
      <div>
        <span className="font-semibold text-gray-500">
